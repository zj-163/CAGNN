...
from torch_geometric.nn import NNConv,GCNConv, GATConv, global_mean_pool, LayerNorm,CGConv
...
class CGCNNModel(torch.nn.Module):
    def __init__(self, num_node_features, num_edge_features, hidden_dim,heads,dropout,nnconv_aggr):
        ...
        self.num_layers=1    # CGConv层的数量
        self.hidden_channels  = hidden_dim   # 隐藏层的维度

        for i in range(self.num_layers):
            
            if i == 0:
                in_channels = num_node_features 
                self.convs.append(CGConv(channels=in_channels, dim=num_edge_features, aggr='add'))
            else:
        ...
        # 全连接层用于数值预测
        # self.fc = torch.nn.Linear(self.hidden_channels , 1)  # 输出维度为1，用于回归任务
        self.fc = torch.nn.Linear(32 , 1)  # 输出维度为1，用于回归任务


    def forward(self, data):
        x, edge_index, edge_attr, batch = data.x, data.edge_index, data.edge_attr, data.batch
        #print(" x.shape,edge_index.shape,edge_attr.shape,batch.shape",  x.shape,edge_index.shape,edge_attr.shape,batch.shape)
        # 逐层进行图卷积
...
def train_allsplit():
    ...
    for i in range(5):
        ...
        savepath=f"./S8_compare_data\\cgcnn/split_by_vacancies/{i}/"
    ...
    df.to_csv("./S8_compare_data\\cgcnn/split_by_vacancies/allresult.csv", index=False)   #地址！！！
    ...
    for i in range(5):
        ...
        print("x_index=",x_index, x_csvpath_list)  
        print("y_index=",y_index, y_csvpath_list)
        savepath=f"./S8_compare_data\\cgcnn/split_by_compounds/{i}/"
    ...
    df = pd.DataFrame(results, columns=["split_type", "fold", "train_loss", "test_loss"])   
    df.to_csv("./S8_compare_data\\cgcnn/split_by_compounds/allresult.csv", index=False)   #地址！！！
    ...
    for element in all_elements:
        ...
        savepath=f"./S8_compare_data\\cgcnn/split_by_elements/{element}/"
    ...
    df = pd.DataFrame(results, columns=["split_type", "fold", "train_loss", "test_loss"])   
    df.to_csv("./S8_compare_data\\cgcnn/split_by_elements/allresult.csv", index=False)   #地址！！！    
    ...
    for i in range(2):
        ...
        savepath=f"./S8_compare_data\\cgcnn/split_by_elements_numbers/{i}/"
    ...
    df = pd.DataFrame(results, columns=["split_type", "fold", "train_loss", "test_loss"])   
    df.to_csv("./S8_compare_data\\cgcnn/split_by_elements_numbers/allresult.csv", index=False)   #地址！！！   
        
    #  最后汇总
    results_df = pd.DataFrame(results, columns=["split_type", "fold", "train_loss", "test_loss"])
    results_df.to_csv("./S8_compare_data\\cgcnn/allresult.csv", index=False)
"""
...
def load_and_test_cv_by_vacancies():
    ...
    csv_path=".\\S8_compare_data\\cgcnn\\split_by_vacancies\\all_vacancies_result.csv"   #！！！！！ 注意地址！！！！！
    index=0
    # 打开CSV文件以追加模式（'a'）打开
    ...
    for i in range(5):
        # 载入模型>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        model = CGCNNModel(num_node_features=32,num_edge_features=2,                                
                                hidden_dim=hidden_dim,heads=heads,
                                dropout=dropout,
                                nnconv_aggr="add")  # 节点特征维度为12  
        model_folderpath=".\\S8_compare_data\\cgcnn\\split_by_vacancies\\"+str(i)     #！！！！！ 注意地址！！！！！
...
def load_and_test_cv_by_compounds():       #！！！！！ 注意名字！！！！！   
    ...
    csv_path=".\\S8_compare_data\\cgcnn\\split_by_compounds\\all_vacancies_result.csv"   #！！！！！ 注意地址！！！！！
    index=0
    # 打开CSV文件以追加模式（'a'）打开
    with open(csv_path, mode='a', newline='') as file:   #newline=''参数的作用是确保在不同操作系统上正确处理换行符。
        writer = csv.writer(file)
    ...
    for i in range(5):
        ...
        model = CGCNNModel(num_node_features=32,num_edge_features=2,                                
                                ...
                                dropout=dropout,
                                nnconv_aggr="add")  # 节点特征维度为12  
        model_folderpath=".\\S8_compare_data\\cgcnn\\split_by_compounds\\"+str(i)     #！！！！！ 注意地址！！！！！
        model_path=next(os.path.join(model_folderpath, f) for f in os.listdir(model_folderpath) if f.endswith("modeldict.pth"))
        print(model_path)
        model.load_state_dict(torch.load(model_path,weights_only=False,map_location=torch.device(device)))
        model.to(device)
...
def load_and_test_cv_by_elements():       #！！！！！ 注意名字！！！！！   
    ...
    csv_path=".\\S8_compare_data\\cgcnn\\split_by_elements\\all_vacancies_result.csv"   #！！！！！ 注意地址！！！！！
    index=0
    # 打开CSV文件以追加模式（'a'）打开
    with open(csv_path, mode='a', newline='') as file:   #newline=''参数的作用是确保在不同操作系统上正确处理换行符。
        writer = csv.writer(file)
    ...
    for element in elements_contianed:
        ...
        model = CGCNNModel(num_node_features=32,num_edge_features=2,                                
                                ...
                                dropout=dropout,
                                nnconv_aggr="add")  # 节点特征维度为12  
        model_folderpath=".\\S8_compare_data\\cgcnn\\split_by_elements\\"+str(element)     #！！！！！ 注意地址！！！！！
        model_path=next(os.path.join(model_folderpath, f) for f in os.listdir(model_folderpath) if f.endswith("modeldict.pth"))
        print(model_path)
        model.load_state_dict(torch.load(model_path,weights_only=False,map_location=torch.device(device)))
        model.to(device)
...
def load_and_test_cv_by_elements_numbers():       #！！！！！ 注意名字！！！！！   
    ...
    csv_path=".\\S8_compare_data\\cgcnn\\split_by_elements_numbers\\all_vacancies_result.csv"   #！！！！！ 注意地址！！！！！
    index=0
    # 打开CSV文件以追加模式（'a'）打开
    with open(csv_path, mode='a', newline='') as file:   #newline=''参数的作用是确保在不同操作系统上正确处理换行符。
        writer = csv.writer(file)
    ...
    for i in range(1,4):
        ...
        model = CGCNNModel(num_node_features=32,num_edge_features=2,                                
                                ...
                                dropout=dropout,
                                nnconv_aggr="add")  # 节点特征维度为12  
        model_folderpath=".\\S8_compare_data\\cgcnn\\split_by_elements_numbers\\"+str(i)     #！！！！！ 注意地址！！！！！
        model_path=next(os.path.join(model_folderpath, f) for f in os.listdir(model_folderpath) if f.endswith("modeldict.pth"))
        print(model_path)
        model.load_state_dict(torch.load(model_path,weights_only=False,map_location=torch.device(device)))
        model.to(device)
