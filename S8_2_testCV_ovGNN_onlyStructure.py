...
def load_and_test_cv_by_vacancies():
    ...
    csv_path=".\\S8_compare_data\\ovgnn_only_structure\\split_by_vacancies\\all_vacancies_result.csv"   #！！！！！ 注意地址！！！！！
    index=0
    # 打开CSV文件以追加模式（'a'）打开
    ...
    for i in range(5):
        ...
        model = GNNWithEdgeFeatures(num_node_features=1,num_edge_features=2,                                
                                ...
                                dropout=dropout,
                                nnconv_aggr="add")  # 节点特征维度为12  
        model_folderpath=".\\S8_compare_data\\ovgnn_only_structure\\split_by_vacancies\\"+str(i)     #！！！！！ 注意地址！！！！！
...
def load_and_test_cv_by_compounds():       #！！！！！ 注意名字！！！！！   
    ...
    csv_path=".\\S8_compare_data\\ovgnn_only_structure\\split_by_compounds\\all_vacancies_result.csv"   #！！！！！ 注意地址！！！！！
    index=0
    # 打开CSV文件以追加模式（'a'）打开
    with open(csv_path, mode='a', newline='') as file:   #newline=''参数的作用是确保在不同操作系统上正确处理换行符。
        writer = csv.writer(file)
    ...
    for i in range(5):
        ...
        model = GNNWithEdgeFeatures(num_node_features=1,num_edge_features=2,                                
                                ...
                                dropout=dropout,
                                nnconv_aggr="add")  # 节点特征维度为12  
        model_folderpath=".\\S8_compare_data\\ovgnn_only_structure\\split_by_compounds\\"+str(i)     #！！！！！ 注意地址！！！！！
        model_path=next(os.path.join(model_folderpath, f) for f in os.listdir(model_folderpath) if f.endswith("modeldict.pth"))
        print(model_path)
        model.load_state_dict(torch.load(model_path,weights_only=False,map_location=torch.device(device)))
        model.to(device)
...
def load_and_test_cv_by_elements():       #！！！！！ 注意名字！！！！！   
    ...
    csv_path=".\\S8_compare_data\\ovgnn_only_structure\\split_by_elements\\all_vacancies_result.csv"   #！！！！！ 注意地址！！！！！
    index=0
    # 打开CSV文件以追加模式（'a'）打开
    with open(csv_path, mode='a', newline='') as file:   #newline=''参数的作用是确保在不同操作系统上正确处理换行符。
        writer = csv.writer(file)
    ...
    for element in elements_contianed:
        ...
        model = GNNWithEdgeFeatures(num_node_features=1,num_edge_features=2,                                
                                ...
                                dropout=dropout,
                                nnconv_aggr="add")  # 节点特征维度为12  
        model_folderpath=".\\S8_compare_data\\ovgnn_only_structure\\split_by_elements\\"+str(element)     #！！！！！ 注意地址！！！！！
        model_path=next(os.path.join(model_folderpath, f) for f in os.listdir(model_folderpath) if f.endswith("modeldict.pth"))
        print(model_path)
        model.load_state_dict(torch.load(model_path,weights_only=False,map_location=torch.device(device)))
        model.to(device)
...
def load_and_test_cv_by_elements_numbers():       #！！！！！ 注意名字！！！！！   
    ...
    csv_path=".\\S8_compare_data\\ovgnn_only_structure\\split_by_elements_numbers\\all_vacancies_result.csv"   #！！！！！ 注意地址！！！！！
    index=0
    # 打开CSV文件以追加模式（'a'）打开
    with open(csv_path, mode='a', newline='') as file:   #newline=''参数的作用是确保在不同操作系统上正确处理换行符。
        writer = csv.writer(file)
    ...
    for i in range(1,4):
        ...
        model = GNNWithEdgeFeatures(num_node_features=1,num_edge_features=2,                                
                                ...
                                dropout=dropout,
                                nnconv_aggr="add")  # 节点特征维度为12  
        model_folderpath=".\\S8_compare_data\\ovgnn_only_structure\\split_by_elements_numbers\\"+str(i)     #！！！！！ 注意地址！！！！！
        model_path=next(os.path.join(model_folderpath, f) for f in os.listdir(model_folderpath) if f.endswith("modeldict.pth"))
        print(model_path)
        model.load_state_dict(torch.load(model_path,weights_only=False,map_location=torch.device(device)))
        model.to(device)
